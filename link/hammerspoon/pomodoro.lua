...
local function pom_disable()
  ...
  elseif (pom_disable_count == 1) then
    ...
    hs.notify.new({
        title='Pomodoro',
        informativeText='Pomodoro paused.'
      }):send()
  elseif (pom_disable_count >= 2) then
    hs.notify.new({
        title='Pomodoro',
        informativeText='Pomodoro canceled.'
      }):send()
  ...
  if pom_is_active == false then
    ...
    if (pom_time_left <= 0 ) then
      ...
      if pom_curr_active_type == "Work" then 
        ...
        hs.notify.new(function() pom_time_left = 0 end, {
          title="Pomodoro", 
        ...
        -- pom_timer:start()
          ...
          hs.notify.new(function() pom_enable() end, {
            title="Pomodoro", 
...
hs.hotkey.bind({"cmd", "alt", "shift"}, '0', function() pom_disable() end)
hs.hotkey.bind({"cmd", "alt", "shift"}, '9', function() pom_enable() end)
hs.hotkey.bind({"cmd", "alt", "shift"}, '8', function() pom_time_left = pom_15min_period_sec; pom_enable() end)
hs.hotkey.bind({"cmd", "alt", "shift"}, '7', function() pom_time_left = pom_time_left - 60 end)
